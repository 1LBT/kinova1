...
# Params (topics and frames)
DEPTH_IMAGE_TOPIC = rospy.get_param('~depth_image_topic', '/camera/depth/image_rect_raw')
CAMERA_INFO_TOPIC = rospy.get_param('~camera_info_topic', '/camera/depth/camera_info')
ROBOT_POSE_TOPIC = rospy.get_param('~robot_pose_topic', '/m1n6s200_driver/out/tool_pose')
GRASP_IMG_TOPIC = rospy.get_param('~grasp_img_topic', 'ggcnn/img/grasp')
GRASP_PLAIN_TOPIC = rospy.get_param('~grasp_plain_img_topic', 'ggcnn/img/grasp_plain')
DEPTH_IMG_TOPIC = rospy.get_param('~depth_img_topic', 'ggcnn/img/depth')
ANG_IMG_TOPIC = rospy.get_param('~ang_img_topic', 'ggcnn/img/ang')
CMD_TOPIC = rospy.get_param('~cmd_topic', 'ggcnn/out/command')

# 打印参数用于调试
rospy.loginfo(f"深度图像话题: {DEPTH_IMAGE_TOPIC}")
rospy.loginfo(f"相机信息话题: {CAMERA_INFO_TOPIC}")
rospy.loginfo(f"机器人位姿话题: {ROBOT_POSE_TOPIC}")

# Output publishers.
grasp_pub = rospy.Publisher(GRASP_IMG_TOPIC, Image, queue_size=1)
grasp_plain_pub = rospy.Publisher(GRASP_PLAIN_TOPIC, Image, queue_size=1)
depth_pub = rospy.Publisher(DEPTH_IMG_TOPIC, Image, queue_size=1)
ang_pub = rospy.Publisher(ANG_IMG_TOPIC, Image, queue_size=1)
cmd_pub = rospy.Publisher(CMD_TOPIC, Float32MultiArray, queue_size=1)
...
try:
    ...
    fy = K[4]
    cy = K[5]
    rospy.loginfo(f"成功获取相机内参: fx={fx}, cx={cx}, fy={fy}, cy={cy}")
except rospy.ROSException as e:
    rospy.logerr(f"等待相机信息超时: {e}")
    rospy.logerr("使用默认相机内参")
    # 使用合理的默认值
    fx = 309.019
    cx = 163.5
    fy = 309.019
...
def robot_pos_callback(data):
    ...
    rospy.loginfo_once(f"接收到机器人位姿，当前Z高度: {ROBOT_Z}")
...
def depth_callback(depth_message):
    ...
    rospy.loginfo_once("深度图像回调函数被调用")
    ...
    with TimeIt('Inpaint'):
        ...
        if depth_scale < 1e-6:
            rospy.logwarn_throttle(2.0, "Depth crop is all zeros/NaN, skip this frame.")
            return
    ...
    with TimeIt('Publish'):
        ...
        rospy.loginfo(f"发布抓取命令: x={x:.3f}, y={y:.3f}, z={z:.3f}, ang={ang:.3f}, width={width:.3f}")

# 创建订阅者
rospy.loginfo(f"创建深度图像订阅: {DEPTH_IMAGE_TOPIC}")
depth_sub = rospy.Subscriber(DEPTH_IMAGE_TOPIC, Image, depth_callback, queue_size=1)
rospy.loginfo("深度图像订阅者创建成功")

rospy.loginfo(f"创建机器人位姿订阅: {ROBOT_POSE_TOPIC}")
robot_pos_sub = rospy.Subscriber(ROBOT_POSE_TOPIC, PoseStamped, robot_pos_callback, queue_size=1)
rospy.loginfo("机器人位姿订阅者创建成功")

rospy.loginfo("GGCNN节点初始化完成，开始运行...")
